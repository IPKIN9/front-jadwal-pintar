...
export default {
  getAll(params: {limit: number; page: number; search: string; jurusan_id: number | null; orderBy: string; sort: string}) {
    return server(baseUrl).get(
      `${endPoint}?search=${params.search}&jurusan_id=${params.jurusan_id}&limit=${params.limit}&page=${params.page}&orderBy=${params.orderBy}&sort=${params.sort}`, AuthConfig.getToken()
    )
  },
